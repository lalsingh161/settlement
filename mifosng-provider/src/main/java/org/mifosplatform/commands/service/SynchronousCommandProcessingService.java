	...
	private NewCommandSourceHandler findCommandHandler(
		...
		if (wrapper.isConfigurationResource()) {
			...
			if (wrapper.isApproveLoanApplication()) {
				...
				handler = applicationContext.getBean("closeLoanCommandHandler",
						NewCommandSourceHandler.class);
			...
			if (wrapper.isCreate()) {
				handler = applicationContext.getBean(
						"createSavingsAccountCommandHandler",
						NewCommandSourceHandler.class);
			} else if (wrapper.isUpdate()) {
				handler = applicationContext.getBean(
						"updateSavingsAccountCommandHandler",
						NewCommandSourceHandler.class);
			} else if (wrapper.isDelete()) {
				handler = applicationContext.getBean(
						"deleteSavingsAccountCommandHandler",
						NewCommandSourceHandler.class);
			...
			if (wrapper.isCreate()) {
				...
				handler = applicationContext.getBean(
						"renewalOrderCommandHandler",
						NewCommandSourceHandler.class);
			} else if (wrapper.isReconnectOrder()) {
				handler = applicationContext.getBean(
						"reconnectOrderCommandHandler",
						NewCommandSourceHandler.class);
			} else if (wrapper.isRetrackOsdMessageOrder()) {
				handler = applicationContext.getBean(
						"retrackOsdMessageOrderCommandHandler",
						NewCommandSourceHandler.class);
			}
		} else if (wrapper.isOrderPriceResource()) {
			if (wrapper.isUpdatePrice()) {
				handler = applicationContext.getBean(
						"updateOrderPriceCommandHandler",
						NewCommandSourceHandler.class);
			}
		} else if (wrapper.isOneTimeSaleResource()) {
			if (wrapper.isCreate()) {
				handler = applicationContext.getBean(
						"createOneTimeSaleCommandHandler",
						NewCommandSourceHandler.class);
			} else if (wrapper.isUpdate()) {
				handler = applicationContext.getBean(
						"updateOneTimeSaleCommandHandler",
						NewCommandSourceHandler.class);
			} else if (wrapper.isDelete()) {
				handler = applicationContext.getBean(
						"deleteOneTimeSaleCommandHandler",
						NewCommandSourceHandler.class);
			} else if (wrapper.isCalculatePrice()) {
				handler = applicationContext.getBean(
						"calculateOneTimeSalePriceCommandHandler",
						NewCommandSourceHandler.class);
			}
			...
			if (wrapper.isCreate()) {
				handler = applicationContext.getBean(
						"createPaymodeCommandHandler",
						NewCommandSourceHandler.class);
			} else if (wrapper.isUpdate()) {
				handler = applicationContext.getBean(
						"updatePaymodeCommandHandler",
						NewCommandSourceHandler.class);
			} else if (wrapper.isDelete()) {
				handler = applicationContext.getBean(
						"deletePaymodeCommandHandler",
						NewCommandSourceHandler.class);
			} else {
				throw new UnsupportedCommandException(wrapper.commandName());
			}
			...
			if (wrapper.isCreate()) {
				handler = applicationContext.getBean(
						"createBillMasterCommandHandler",
						NewCommandSourceHandler.class);
			} else if (wrapper.isUpdate()) {
				handler = applicationContext.getBean(
						"updateBillMasterCommandHandler",
						NewCommandSourceHandler.class);
			} else if (wrapper.isDelete()) {
				handler = applicationContext.getBean(
						"deleteBillMasterCommandHandler",
						NewCommandSourceHandler.class);
			} else {
				throw new UnsupportedCommandException(wrapper.commandName());
			}
			...
			if (wrapper.isCreate()) {
				handler = applicationContext.getBean(
						"createInventoryItemsCommandHandler",
						NewCommandSourceHandler.class);
			}
		} else if (wrapper.isGrnResource()) {
			if (wrapper.isCreate()) {
				handler = applicationContext.getBean(
						"createInventoryGrnCommandHandler",
						NewCommandSourceHandler.class);
			}
		} else if (wrapper.isAllocateHardwareResource()) {
			if (wrapper.isCreate()) {
				handler = applicationContext.getBean(
						"createInventoryItemAllocationCommandHandler",
						NewCommandSourceHandler.class);
			}
		} else if (wrapper.isUploadStatusResource()) {
			if (wrapper.isCreate()) {
				handler = applicationContext.getBean(
						"uploadStatusWritePlatformService",
						NewCommandSourceHandler.class);
			}
		} else if (wrapper.isChargeCodeResource()) {
			if (wrapper.isCreate()) {
				handler = applicationContext.getBean(
						"createChargeCodeCommandHandler",
						NewCommandSourceHandler.class);
			}
			if (wrapper.isUpdateChargeCode()) {
				handler = applicationContext.getBean(
						"updateChargeCodeCommandHandler",
						NewCommandSourceHandler.class);
			}
		} else if (wrapper.isTaxMapResource()) {
			if (wrapper.isCreate()) {
				handler = applicationContext.getBean(
						"createTaxMapCommandHandler",
						NewCommandSourceHandler.class);
			}
			if (wrapper.isUpdateTaxMap()) {
				handler = applicationContext.getBean(
						"updateTaxMapCommandHandler",
						NewCommandSourceHandler.class);
			}
		} else if (wrapper.isTicketResource()) {
			if (wrapper.isCreateTicket()) {
				handler = applicationContext.getBean(
						"createTicketMasterCommandHandler",
						NewCommandSourceHandler.class);
			} else if (wrapper.isUpdateTicket()) {
				handler = applicationContext.getBean(
						"updateTicketMasterCommandHandler",
						NewCommandSourceHandler.class);
			} else if (wrapper.isCloseTicket()) {
				handler = applicationContext.getBean(
						"deleteTicketMasterCommandHandler",
						NewCommandSourceHandler.class);
			} else {
				throw new UnsupportedCommandException(wrapper.commandName());
			}
			...
			if (wrapper.isCreateEvent()) {
				handler = applicationContext.getBean(
						"createEventMasterCommandHandler",
						NewCommandSourceHandler.class);
			} else if (wrapper.isUpdateEvent()) {
				handler = applicationContext.getBean(
						"updateEventMasterCommandHandler",
						NewCommandSourceHandler.class);
			} else if (wrapper.isCloseEvent()) {
				handler = applicationContext.getBean(
						"closeEventMasterCommandHandler",
						NewCommandSourceHandler.class);
			} else {
				throw new UnsupportedCommandException(wrapper.commandName());
			}
			...
			if (wrapper.isCreateEventPrice()) {
				handler = applicationContext.getBean(
						"createEventPriceCommandHandler",
						NewCommandSourceHandler.class);
			...
			if (wrapper.isCreateEventOrder()) {
				handler = applicationContext.getBean(
						"createEventOrderCommandHandler",
						NewCommandSourceHandler.class);
			} else if (wrapper.isUpdateEventPrice()) {
				handler = applicationContext.getBean(
						"updateEventPriceCommandHandler",
				...
				handler = applicationContext.getBean(
						"updateEventOrderPriceCommandHandler",
						NewCommandSourceHandler.class);
			} else if (wrapper.isCloseEventPrice()) {
				handler = applicationContext.getBean(
						"closeEventPriceCommandHandler",
			...
			if (wrapper.isCreateBillingMessage()) {
				handler = applicationContext.getBean(
						"createBillingMessageTemplateCommandHandler",
						NewCommandSourceHandler.class);
			} else if (wrapper.isUpdateBillingMessage()) {
				handler = applicationContext.getBean(
						"updateBillingMessageTemplateCommandHandler",
						NewCommandSourceHandler.class);
			} else if (wrapper.isDeleteBillingMessage()) {
				handler = applicationContext.getBean(
						"deleteBillingMessageTemplateCommandHandler",
						NewCommandSourceHandler.class);
			} else if (wrapper.isCreateMessageData()) {
				handler = applicationContext.getBean(
						"createMessageDataCommandHandler",
						NewCommandSourceHandler.class);
			} else {
				throw new UnsupportedCommandException(wrapper.commandName());
			}
			...
			handler = applicationContext.getBean(
					"addInventoryItemAllocationCommandHandler",
					NewCommandSourceHandler.class);
		} else if (wrapper.isMediaAssetResource()) {
			if (wrapper.isCreateMediaAsset()) {
				handler = applicationContext.getBean(
						"createMediaAssetCommandHandler",
						NewCommandSourceHandler.class);
			...
			if (wrapper.isCreate()) {
				handler = applicationContext.getBean(
						"createBatchJobCommandHandler",
						NewCommandSourceHandler.class);
			}
		} else if (wrapper.isClientProspect()) {
			if (wrapper.isCreate()) {
				handler = applicationContext.getBean(
						"createClientProspectCommandHandler",
						NewCommandSourceHandler.class);
			}
			if (wrapper.isUpdateClientProspect()) {
				handler = applicationContext.getBean(
						"updateClientProspectCommandHandler",
						NewCommandSourceHandler.class);
			}
			if (wrapper.isDeleteClientProspect()) {
				handler = applicationContext.getBean(
						"deleteClientProspectCommandHandler",
						NewCommandSourceHandler.class);
			}
			if (wrapper.isConvertClientProspect()) {
				handler = applicationContext.getBean(
						"createClientProspectConvertCommandHandler",
						NewCommandSourceHandler.class);
			}
			if (wrapper.isEditClientProspect()) {
				handler = applicationContext.getBean(
						"editClientProspectCommandHandler",
						NewCommandSourceHandler.class);
			}
		} else if (wrapper.isClientBalance()) {
			if (wrapper.isCreate()) {
				handler = applicationContext.getBean(
						"createClientBalanceCommandHandler",
						NewCommandSourceHandler.class);
			}
		} else if (wrapper.isOwnedHardware()) {
			if (wrapper.isCreate()) {
				handler = applicationContext.getBean(
						"createOwnedHardwareCommandHandler",
						NewCommandSourceHandler.class);
			}
		} else if (wrapper.isSchedulling()) {
			if (wrapper.isCreate()) {
				handler = applicationContext.getBean(
						"createBatchJobSchedulingCommandHandler",
						NewCommandSourceHandler.class);
			} else if (wrapper.isUpdate()) {
				handler = applicationContext.getBean(
						"updateJobParameterCommandHandler",
						NewCommandSourceHandler.class);
			}
		} else if (wrapper.isCountryCurrencyResource()) {
			if (wrapper.isCreate()) {
				handler = applicationContext.getBean(
						"createCountryCurrencyCommandHandler",
						NewCommandSourceHandler.class);
			}
			if (wrapper.isUpdateCurrencyConfig()) {
				handler = applicationContext.getBean(
						"updateCountryCurrencyCommandHandler",
						NewCommandSourceHandler.class);
			}
			if (wrapper.isDeleteCurrencyConfig()) {
				handler = applicationContext.getBean(
						"deleteCountryCurrencyCommandHandler",
						NewCommandSourceHandler.class);
			}
		} else if (wrapper.isEpgProgramGuide()) {
			if (wrapper.isCreate()) {
				handler = applicationContext.getBean(
						"createEpgProgramGuideCommandHandler",
						NewCommandSourceHandler.class);
			}
			...
			if (wrapper.isCreateMRN()) {
				handler = applicationContext.getBean(
						"createMRNDetailsCommandHandler",
						NewCommandSourceHandler.class);
			} else if (wrapper.moveMRN()) {
				handler = applicationContext.getBean(
						"createMRNDetailsMoveCommandHandler",
						NewCommandSourceHandler.class);
			}
			...
			if (wrapper.isCreateRandomGenerator()) {
				handler = applicationContext.getBean(
						"createRandomGeneratorCommandHandler",
						NewCommandSourceHandler.class);
			}
			...
			if (wrapper.isCreateDeductionCodes()) {
				handler = applicationContext.getBean(
						"createMasterDeductionCodesCommandHandler",
						NewCommandSourceHandler.class);
			} else if (wrapper.isUpdateDeductionCodes()) {
				handler = applicationContext.getBean(
						"updateMasterDeductionCodesCommandHandler",
						NewCommandSourceHandler.class);
			} else if (wrapper.isDeleteDeductionMaster()) {
				handler = applicationContext.getBean(
						"deleteMasterDeductionCommandHandler",
						NewCommandSourceHandler.class);
			}
			...
			if (wrapper.isCreatePartnerGame()) {
				handler = applicationContext.getBean(
						"createPartnerGameCommandHandler",
						NewCommandSourceHandler.class);
			} else if (wrapper.isUpdatePartnerGame()) {
				handler = applicationContext.getBean(
						"updatePartnerGameCommandHandler",
						NewCommandSourceHandler.class);
			} else if (wrapper.isCreateRevenueSettlement()) {
				handler = applicationContext.getBean(
						"createRevenueSettlementCommandHandler",
						NewCommandSourceHandler.class);
			} else if (wrapper.isDeletePartnerGame()) {
				handler = applicationContext.getBean(
						"deletePartnerGameCommandHandler",
						NewCommandSourceHandler.class);
			}
