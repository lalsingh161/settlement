...
public class MediaAssetDetailsReadPlatformServiceImpl implements MediaAssetDetailsReadPlatformService {
	...
	public String attributeDataSchema() {
		...
		public List<MediaAssetLocationDetails> retrieveMediaAssetLocationData(
				String codeValue) {
			MALMapper mapper = new MALMapper();
			String sql = "select mcv.code_value as formatType from m_code_value mcv inner join m_code mc on mc.id=mcv.code_id where mc.code_name = ?";
			return jdbcTemplate.query(sql, mapper,new Object[] {codeValue});
		}
		
		@Override
		public Long retriveCategory(Long mediaId) {
			String sql = "select category_id as categoryId from b_media_asset where id = ?";
			return jdbcTemplate.queryForLong(sql, mediaId);
		}
		...
		private static final class MALMapper implements
		RowMapper<MediaAssetLocationDetails> {
			...
			public MediaAssetLocationDetails mapRow(ResultSet rs, int rowNum)
					throws SQLException {
				String formatType = rs.getString("formatType");
				return new MediaAssetLocationDetails(formatType);
			}
