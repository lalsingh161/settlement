...
public class EventPricingWritePlatformServiceImpl implements
	...
	private final static Logger logger = (Logger) LoggerFactory.getLogger(EventPricingWritePlatformServiceImpl.class);
	...
	private void handleDataIntegrityIssues(final JsonCommand element, final DataIntegrityViolationException dve) {
       ...
       if (realCause.getMessage().contains("UK_PRICING_COMPKEY")){
       	throw new PlatformDataIntegrityException("event.pricing.price.already.exist", "event.pricing.price.already.exist", "event.pricing.price.already.exist","");    	
       }
