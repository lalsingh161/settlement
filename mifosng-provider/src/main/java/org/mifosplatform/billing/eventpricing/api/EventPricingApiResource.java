...
public class EventPricingApiResource {
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	public String retrievePriceTemplateGameData(@QueryParam("eventId") Long eventId,@Context UriInfo uriInfo) {
		Set<String> responseParameters = ApiParameterHelper.extractFieldsForResponseIfProvided(uriInfo.getQueryParameters());
		responseParameters.addAll(RESPONSE_PARAMETERS);
		EventPricingData templateData = handleGameTemplateRelatedData(eventId);
		final ApiRequestJsonSerializationSettings settings = apiRequestParameterHelper.process(uriInfo.getQueryParameters());
		return this.apiJsonSerializer.serialize(settings, templateData, RESPONSE_PARAMETERS);
	}
	
	
	public EventPricingData handleGameTemplateRelatedData(Long eventId) {
		Collection<MCodeData> optType = this.codeReadPlatformService.getCodeValue("MediaGameDescription");
		...
		Long categoryId = this.assetDetailsReadPlatformService.retriveCategory(details.get(0).getMediaId());
				
		Collection<MCodeData> format = this.codeReadPlatformService.getCodeValue("Deck Type");
		
		List<DiscountMasterData> discountdata = this.priceReadPlatformService.retrieveDiscountDetails();
		
		
		
 		EventPricingData pricingData = new EventPricingData(optType,format,discountdata,clientType,eventId,categoryId);
		return pricingData;
	}
	...
	public EventPricingData handleTemplateRelatedData(Long eventId) {
		...
		List<ClientTypeData> clientType = this.eventPricingReadService.clientType();
		Long categoryId = this.assetDetailsReadPlatformService.retriveCategory(details.get(0).getMediaId());
		List<MediaAssetLocationDetails> format = null;
		if(categoryId!=null){
			if(categoryId == 999991){
				format = this.assetDetailsReadPlatformService.retrieveMediaAssetLocationData("MediaFormat");
			}else{
				format = this.assetDetailsReadPlatformService.retrieveMediaAssetLocationData(details.get(0).getMediaId());
			}
		}else{
			format = this.assetDetailsReadPlatformService.retrieveMediaAssetLocationData(details.get(0).getMediaId());
		}
