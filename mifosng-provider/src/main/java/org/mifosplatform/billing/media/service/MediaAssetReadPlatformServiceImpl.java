...
import org.mifosplatform.billing.media.data.GameMediaDetailsData;
...
import org.mifosplatform.billing.media.data.SettlementData;
...
public class MediaAssetReadPlatformServiceImpl implements MediaAssetReadPlatformService {
	...
	public MediaAssetData mapRow(ResultSet rs, int rowNum)
		...
		return new MediaAssetData(mediaId,mediaTitle,status,releaseDate,rating,mediaCategory,partnerName);
	}
	public String mediaAssestDataSchema() {
		return "m.id AS id, m.title AS mediaTitle, m.status AS status,m.release_date as releaseDate,m.rating as rating,m.media_category as mediaCategory,"+
			    ""+"pa.partner_name as partnerName FROM b_media_asset m inner join bp_account pa on m.content_provider = pa.id	where m.is_deleted = 'N'";
			    /*" m.id AS id,  m.title AS mediaTitle,m.status AS status,m.release_date as releaseDate,m.rating as rating, m.media_category as mediaCategory"
	...
	public MediaAssetData mapRow(ResultSet rs, int rowNum)
		...
		final String contentProvider1 = rs.getString("contentProvider");
		 Long contentProvider=Long.parseLong(contentProvider1);
		...
		final String mediaCategoryG = rs.getString("mediaCategory");
		return new MediaAssetData(mediaId,mediatitle,type,genre,catageoryId,releaseDate,subject,overview,image,contentProvider,
				rated,rating,ratingCount,status,duration,mediaCategoryG);
	}
	public String scheme() {

		return " m.id as id,m.title as title, m.media_category as mediaCategory, m.type as type,m.category_id as catageoryId,m.genre as genre,m.release_date as releaseDate,"
		...
		@Override
		public List<SettlementData> retrieveSettlementData(Long id) {
			try{
			
			SettlementDataMapper mapper = new SettlementDataMapper();
			String sql = "Select " + mapper.scheme() + "  bs.mediaId=?";
	...
	private static final class SettlementDataMapper implements RowMapper<SettlementData> {
		...
		@Override
		public SettlementData mapRow(ResultSet rs, int rowNum)
				throws SQLException {
	
			final Long id = rs.getLong("id");
			final Long mediaId = rs.getLong("mediaId");
			...
			if(mediaType.equalsIgnoreCase("P")){
				mediaType="Percentage";
			}else if(mediaType.equalsIgnoreCase("F")){
				mediaType="Flat";
			}
			
			
			return new SettlementData( id, mediaId, description,  category,
					source,  sequence,  mediaType, amount);
		}
		public String scheme() {
	
			return " bs.ID as id,bs.Description as description,bs.Category as category,"
					+ "bs.source as source,bs.seq as sequence,bs.MediaId as mediaId,"
					+ "bs.Type as mediaType,bs.Amt as amount FROM b_settlement bs WHERE ";
			}
		}
...
private static final class MediaLocationDataMapper implements RowMapper<MediaLocationData> {
		...
		@Override
		public List<SettlementData> retriveSettlementDetails(Long mediaId) {
			final String sql = "select s.ID as id, s.MediaId as mediaId, s.Amt as amount, s.Source as source, s.Category as category, s.Description as contentProvider,s.Type as mediaType, s.Seq as sequence from b_settlement s where s.MediaId = ?";
			SettlementMapper mapper = new SettlementMapper();
			return jdbcTemplate.query(sql,mapper,new Object[]{mediaId});
		}
		
		static final class SettlementMapper implements RowMapper<SettlementData>{
			@Override
			public SettlementData mapRow(ResultSet rs, int rowNum)
					throws SQLException {
				final Long id = rs.getLong("id");
				final BigDecimal amount = rs.getBigDecimal("amount");
				final String category = rs.getString("category");
				final String mediaContentProvider = rs.getString("contentProvider");
				final String mediaType = rs.getString("mediaType");
				final Long sequence = rs.getLong("sequence");
				final Long mediaId = rs.getLong("mediaId");
				final String source = rs.getString("source");
				return new SettlementData(id, amount, category, mediaContentProvider, mediaType.equalsIgnoreCase("F")?"Flat":"Percentage", sequence,mediaId,source);
			}
		}
		...
		private final static class ContentProvider implements RowMapper<GameMediaDetailsData>{
			@Override
			public GameMediaDetailsData mapRow(ResultSet rs, int rowNum)
					throws SQLException {
				final Long id = rs.getLong("id");
				final String partnerName = rs.getString("partnerName");
				return new GameMediaDetailsData(id,partnerName);
			}
		}
		
		@Override
		public String getPartnerName(Long partnerId) {
			String sql = "select pa.partner_name as partnerName from bp_account pa where pa.id = ?";
			PartnerNameMapper mapper = new PartnerNameMapper();
			return jdbcTemplate.queryForObject(sql, mapper,new Object[]{partnerId});
			
		}
		private static final class PartnerNameMapper implements RowMapper<String>{
			@Override
