...
public final class ClientDataValidator {
   ...
   //     final Boolean active = fromApiJsonHelper.extractBooleanNamed(ClientApiConstants.activeParamName, element);
        ...
        if (fromApiJsonHelper.parameterExists(ClientApiConstants.licenseFreeParamName, element)) {
            final String licenseFree = fromApiJsonHelper.extractStringNamed(ClientApiConstants.licenseFreeParamName, element);
            baseDataValidator.reset().parameter(ClientApiConstants.licenseFreeParamName).value(licenseFree).notNull()
                    .notExceedingLengthOf(5);
        }
        if (fromApiJsonHelper.parameterExists(ClientApiConstants.royaltyValueParamName, element)) {
            final String royaltyValue = fromApiJsonHelper.extractStringNamed(ClientApiConstants.royaltyValueParamName, element);
            baseDataValidator.reset().parameter(ClientApiConstants.royaltyValueParamName).value(royaltyValue).notNull()
                    .notExceedingLengthOf(5);
        }
